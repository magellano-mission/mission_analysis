...
data.Nsat = 21;                 % number of satellites Walker constellation
data.inc = deg2rad(45);         % inclination [rad]
...
% Polar extension in case of chemical propulsion for SM
data.PE = false;                        % add or not the polar extention
if data.PE == true
    data.Nsat_pol = 6;
    data.inc_pol = deg2rad(90);         % [rad]
...
data.alt = 0;                      % altitude at which evaluate the coverage ( surface level = 0 )              
% data.alt = 1.5150e+003;            % RS altitude
% data.alt = 4.0150e+003;            % ECS altitude
data.N_orbits = 2.1435;            % number of orbits in the simulation

% data.study = "DOP";                % NS dilution of precision study
data.study = "coverage";           % study of the coverage
