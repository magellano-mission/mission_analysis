...
figure; plot(a_J2, dv_h, 'LineWidth', 2)
xlabel('radius of lower orbit [km]'), ylabel('\Delta_v [km/s]');
title('homhann to reach RNS')
...
figure; hold on
for i = 1:N
    t = 2*(a_GNSS - a_J2(i))./(at*a_J2(i)*sqrt(a_GNSS/mi))/86400/30;
    plot(T, t, 'LineWidth', 2)
end

xlabel('on-board continuos thrust [N]'), ylabel('tof [months]');
title('low-thrust to reach RNS')
legend(horzcat(strcat('a = ', " ", string(a_J2))))
