...
d_plan = fzero(@(d) find_d(d, ABC, muS, L, r2norm, th_dot2, gamma2, TOF, n_int), 0, opts);
...
[I, ~, TH] = TOFcomp(d_plan, ABC, muS, L, r2norm, th_dot2, gamma2, n_int);
%TH: [n_int,1]
TH2 = TH.^2; TH3 = TH.^3;
TH4 = TH.^4; TH5 = TH.^5;
TH6 = TH.^6;

r = 1./(a_plan + b_plan*TH + c_plan*TH2 + d_plan*TH3 + e_plan*TH4 + f_plan*TH5 + g_plan*TH6);
...
if isreal(theta_dot)   
    ...
    time = zeros(size(TH));
    for i = 2:n_int
        time(i) = time(i-1) + dTOF(i-1) + 4*dTOFm(i-1) + dTOF(i);
    end
    time = time*hh/6;
