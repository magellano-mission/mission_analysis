...
alpha = data.optim.alpha;
beta = data.optim.beta;
...
for nmc = 1:NMC
   ...
   TMC(:,nmc)        = THSNS1 + sigma_T*randn(1,data.n_int);
   %propagation
   
  [XMC(:,:,nmc)] = propagationADMC(TMC(:,nmc), alphaMC(:,nmc), betaMC(:,nmc), X0MC, muS, time1, data);
...
for nmc = 1:NMC
    rrMCtemp = squeeze(rrMC(:,:,nmc));
    dist(nmc) = norm(rrMCtemp(end,:) - RM(end,:));
end
max(dist)
min(dist)